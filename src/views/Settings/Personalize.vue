<template>
  <div class="mail-settings">
    <a-form
      ...
      :label-col="labelCol"
    >
      <a-form-item label="风格选择" :wrapperCol="wrapperCol">
        <a-select
      ...
      <a-form-item v-if="formState.style === 'fresh'" label="背景选择">
        <ul class="bglist">
          <li v-for="(item, index) in imgs" :key="index" :class="{ active: formState.current === item }" @click="chooseImg(item)">
            <img :src="CONFIG.REQ_URL + item" alt="" />
            <CloseCircleOutlined v-if="formState.current !== item" class="icon" @click.stop="deleteImg(item, index)"/>
          </li>
      ...
      <a-form-item label="字体选择" :wrapperCol="wrapperCol">
        <a-select
          v-model:value="formState.font"
          :options="fontOptions"
        >
        </a-select>
      </a-form-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    let formState = ref<PersonalizeSettingsConfig> ({
      style: 'simple',
